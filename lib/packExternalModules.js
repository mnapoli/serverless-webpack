...
const { log, progress, legacy } = require('./log');
...
function removeExcludedModules(modules, packageForceExcludes, log) {
  ...
  if (!_.isEmpty(excludedModules)) {
    if (log) legacy.log(`Excluding external modules: ${_.join(excludedModules, ', ')}`);
    log.notice(`Excluding external modules: ${_.join(excludedModules, ', ')}`);
  }
}
    ...
    if (moduleVersion) {
      ...
      if (nodeModulesRelativeDir) {
        ...
        if (fse.pathExistsSync(customNodeModulesDir)) {
          ...
          legacy.log(`WARNING: ${customNodeModulesDir} dose not exist. Please check nodeModulesRelativeDir setting`);
      ...
      try {
        ...
        if (!_.isEmpty(peerDependencies)) {
          this.options.verbose && legacy.log(`Adding explicit peers for dependency ${module.external}`);
          log.info(`Adding explicit peers for dependency ${module.external}`);
          ...
          if (!_.isEmpty(peerDependenciesMeta)) {
            _.forEach(peerDependencies, (value, key) => {
              if (peerDependenciesMeta[key] && peerDependenciesMeta[key].optional === true) {
                ...
                log.info(`Skipping peers dependency ${key} for dependency ${module.external} because it's optional`);
      ...
      if (!packageJson.devDependencies || !packageJson.devDependencies[module.external]) {
        ...
        if (!moduleVersion) {
          legacy.log(`WARNING: Could not determine version of module ${module.external}`);
...
module.exports = {
  ...
  packExternalModules() {
    ...
    progress.get('webpack').notice('[Webpack] Packing external modules');
    ...
    return BbPromise.try(() => Packagers.get.call(this, this.configuration.packager)).then(packager => {
      ...
      if (!_.isEmpty(packageSections)) {
        this.options.verbose && legacy.log(`Using package.json sections ${_.join(_.keys(packageSections), ', ')}`);
        log.info(`Using package.json sections ${_.join(_.keys(packageSections), ', ')}`);
      }

      // Get first level dependency graph
      this.options.verbose && legacy.log(`Fetch dependency graph from ${packageJsonPath}`);
      log.info(`Fetch dependency graph from ${packageJsonPath}`);
      ...
      return packager.getProdDependencies(path.dirname(packageJsonPath), 1).then(dependencyGraph => {
        ...
        if (this.options.verbose && !_.isEmpty(problems)) {
          legacy.log(`Ignoring ${_.size(problems)} NPM errors:`);
          log.info(`Ignoring ${_.size(problems)} NPM errors:`);
        ...
        if (_.isEmpty(compositeModules)) {
          ...
          legacy.log('No external modules needed');
          log.notice('No external modules needed');
          return BbPromise.resolve();
        }
        ...
        return BbPromise.fromCallback(cb => fse.pathExists(packageLockPath, cb))
          .then(exists => {
            if (exists) {
              legacy.log('Package lock found - Using locked versions');
              log.notice('Package lock found - Using locked versions');
              try {
                ...
                legacy.log(`Warning: Could not read lock file: ${err.message}`);
          ...
          .then(() => {
            const start = _.now();
            legacy.log('Packing external modules: ' + compositeModules.join(', '));
            log.notice('Packing external modules: ' + compositeModules.join(', '));
            return packager
              .install(compositeModulePath, this.configuration.packagerOptions)
              .then(() => {
                this.options.verbose && legacy.log(`Package took [${_.now() - start} ms]`);
                log.info(`Package took [${_.now() - start} ms]`);
                return null;
              })
              .return(stats.stats);
          })
          .mapSeries(compileStats => {
            ...
            return BbPromise.try(() => {
              ...
              .tap(() => {
                this.options.verbose && legacy.log(`Copy modules: ${modulePath} [${_.now() - startCopy} ms]`);
                log.info(`Copy modules: ${modulePath} [${_.now() - startCopy} ms]`);
              })
              .then(() => {
                // Prune extraneous packages - removes not needed ones
                const startPrune = _.now();
                return packager.prune(modulePath, this.configuration.packagerOptions).tap(() => {
                  this.options.verbose && legacy.log(`Prune: ${modulePath} [${_.now() - startPrune} ms]`);
                  log.info(`Prune: ${modulePath} [${_.now() - startPrune} ms]`);
                });
              })
              .then(() => {
                // Prune extraneous packages - removes not needed ones
                ...
                return packager.runScripts(modulePath, _.keys(packageScripts)).tap(() => {
                  this.options.verbose && legacy.log(`Run scripts: ${modulePath} [${_.now() - startRunScripts} ms]`);
                  log.info(`Run scripts: ${modulePath} [${_.now() - startRunScripts} ms]`);
                });
              });
          })
          .return();
