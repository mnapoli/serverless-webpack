...
function setArtifactPath(funcName, func, artifactPath) {
  ...
  this.options.verbose && legacy.log(`Setting artifact for function '${funcName}' to '${artifactPath}'`);
  log.info(`Setting artifact for function '${funcName}' to '${artifactPath}'`);
  ...
  if (semver.lt(version, '1.18.0')) {
    ...
    legacy.log(`${funcName} is packaged by the webpack plugin. Ignore messages from SLS.`);
  } else {
    func.package = {
...
function zip(directory, name) {
  ...
  if (this.configuration.excludeRegex) {
    ...
    if (this.options.verbose) {
      legacy.log(`Excluded ${existingFilesLength - files.length} file(s) based on excludeRegex`);
    }
    log.info(`Excluded ${existingFilesLength - files.length} file(s) based on excludeRegex`);
  ...
  return new BbPromise((resolve, reject) => {
    zipMethod(zipArgs)
      .then(() => {
        ...
        this.options.verbose && legacy.log(`Zip method used: ${zipMethod.name === 'nodeZip' ? 'node' : 'native'}`);
        log.info(`Zip method used: ${zipMethod.name === 'nodeZip' ? 'node' : 'native'}`);
        return null;
      })
      .catch(err => {
        reject(err);
...
module.exports = {
  packageModules() {
    ...
    progress.get('webpack').notice('[Webpack] Packaging modules');
    ...
    return BbPromise.mapSeries(stats.stats, (compileStats, index) => {
      ...
      return zip.call(this, modulePath, filename).tap(() => {
        ...
        log.info(`Zip ${_.isEmpty(entryFunction) ? 'service' : 'function'}: ${modulePath} [${_.now() - startZip} ms]`);
      });
    });
  },

  copyExistingArtifacts() {
    progress.get('webpack').notice('[Webpack] Copying existing artifacts');
    legacy.log('Copying existing artifacts...');
