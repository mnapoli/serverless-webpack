  ...
  validate() {
    ...
    const getEntryExtension = fileName => {
      ...
      if (_.size(sortedFiles) > 1) {
        ...
        log.warning(`More than one matching handlers found for "${fileName}". Using "${_.first(sortedFiles)}"`);
      }
    ...
    this.options.verbose && legacy.log(`Using configuration:\n${JSON.stringify(this.configuration, null, 2)}`);
    log.info(`Using configuration:\n${JSON.stringify(this.configuration, null, 2)}`);
    if (this.configuration.hasLegacyConfig) {
      legacy.log('Legacy configuration detected. Consider to use "custom.webpack" as object (see README).');
      log.warning('Legacy configuration detected. Consider to use "custom.webpack" as object (see README).');
    }
    ...
    if (_.isString(this.webpackConfig)) {
      ...
      try {
        ...
        legacy.log(`Could not load webpack config '${webpackConfigFilePath}'`);
        log.error(`Could not load webpack config "${webpackConfigFilePath}"`);
    ...
    const processConfig = _config => {
      ...
      if (this.options.build === false) {
        ...
        legacy.log('Skipping build and using existing compiled output');
        log.notice('Skipping build and using existing compiled output');
      ...
      if (!this.keepOutputDirectory) {
        this.options.verbose && legacy.log(`Removing ${this.webpackConfig.output.path}`);
        log.info(`Removing ${this.webpackConfig.output.path}`);
      ...
      if (_.has(this.serverless, 'service.package') && this.serverless.service.package.individually) {
        this.options.verbose &&
          legacy.log(`Individually packaging with concurrency at ${this.configuration.concurrency} entries a time.`);
        log.info(`Individually packaging with concurrency at ${this.configuration.concurrency} entries a time.`);
