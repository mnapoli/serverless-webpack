...
const { log, progress, legacy } = require('@serverless/utils/log');

module.exports = {
  wpwatch() {
    if (this.options['webpack-no-watch']) {
      // If we do not watch we will just run an ordinary compile
      legacy.log('Watch disabled by option.');
    ...
    watchProgress.notice('[Webpack] Building with Webpack');
    ...
    const startWatch = callback => {
      ...
      compiler.watch(watchOptions, (err, stats) => {
        ...
        process.env.SLS_DEBUG && legacy.log(`Webpack watch invoke: HASH NEW=${stats && stats.hash} CUR=${lastHash}`);
        log.info(`Webpack watch invoke: HASH NEW=${stats && stats.hash} CUR=${lastHash}`);
